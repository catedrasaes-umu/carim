...
public class GuiNavigationAbstractorListener extends UserTurnPushingListener {
	...
	public GuiNavigationAbstractorListener(EventPool epool, MmiEventsFactory efact, CarimModelInstantiator inst) {
	...
	public void onEvent(int type, Event e) {
		...
		else if (type == event_t.CLICK_VALUE || 
				type == event_t.LONGCLICK_VALUE){
		...
		else if (type == event_t.SCROLL_VALUE){
	...
	public void doWork(Event e) {
		//reset counters
		//_resetCounters();
		LOG.debug(e.getTimestamp() + "\t   L1 :: GuiNavigationAbstractorL: Work done.");
	}

	@Override
	public LinkedList<Integer> registerTypes() {
		LinkedList<Integer> auxl = new java.util.LinkedList<Integer>();
		auxl.add(event_t.TOUCH_VALUE);
		auxl.add(event_t.CLICK_VALUE);
		auxl.add(event_t.LONGCLICK_VALUE);
		auxl.add(event_t.SCROLL_VALUE);
		auxl.add(event_t.MOVE_VALUE);
